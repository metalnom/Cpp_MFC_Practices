...
void CDateTimeDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	GetDlgItem(IDC_STATIC_Y)->SetWindowTextW((LPCTSTR)strY);

	strM.Format(_T("%d 월"), gct.GetMonth());
	GetDlgItem(IDC_STATIC_M)->SetWindowTextW((LPCTSTR)strM);

	strD.Format(_T("%d 일"), gct.GetDay());
	GetDlgItem(IDC_STATIC_D)->SetWindowTextW((LPCTSTR)strD);

	if (gct.GetHour() >= 12) {
		strN.Format(_T("오후"));
		GetDlgItem(IDC_STATIC_N)->SetWindowTextW((LPCTSTR)strN);
		if (gct.GetHour() == 12) {
		...
		GetDlgItem(IDC_STATIC_T)->SetWindowTextW((LPCTSTR)strT);
	}
	else {
		strN.Format(_T("오전"));
		GetDlgItem(IDC_STATIC_N)->SetWindowTextW((LPCTSTR)strN);
	...
	int week = gct.GetDayOfWeek();

	switch (week) {
	case 0:	strW = "일요일"; break;
	case 1:	strW = "월요일"; break;
	case 2:	strW = "화요일"; break;
	case 3: strW = "수요일"; break;
	case 4: strW = "목요일"; break;
	case 5: strW = "금요일"; break;
	case 6: strW = "토요일"; break;	
	}
	GetDlgItem(IDC_STATIC_W)->SetWindowTextW((LPCTSTR)strW);
