...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_EDIT_K, m_nScoreK);
	DDX_Text(pDX, IDC_EDIT_E, m_nScoreE);
	DDX_Text(pDX, IDC_EDIT_M, m_nScoreM);
	DDX_Text(pDX, IDC_EDIT_MEAN, m_nMean);
...
void CCalcExDlg::OnClickedButtonIn()
{
	...
	sum = _wtof(m_nScoreK) + _wtof(m_nScoreE) + _wtof(m_nScoreM);
	mean = sum / 3;
	...
	sprintf_s(temp, "%2.f", sum);
	m_nSum = temp;

	sprintf_s(temp, "%2.f", mean);
	m_nMean = temp;
