...
//	ON_COMMAND(IDC_BUTTON_SEND, &CAboutDlg::OnButtonSend)
END_MESSAGE_MAP()
...
CSendMsgDlg::CSendMsgDlg(CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(CSendMsgDlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_BUTTON_SEND, &CSendMsgDlg::OnClickedButtonSend)
	ON_WM_LBUTTONDOWN()
	ON_WM_RBUTTONDOWN()
END_MESSAGE_MAP()
...
void CSendMsgDlg::OnClickedButtonSend()
{
...
void CSendMsgDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (hWnd)
	{
		...
		mouseStatus.x = point.x;
		mouseStatus.y = point.y;
		strcpy_s(mouseStatus.pTmp, "Left button click");
		msg.cbData = sizeof(MOUSESTATUS);
		msg.dwData = UM_MOUSESTATUS;
		msg.lpData = (PVOID)&mouseStatus;
...
void CSendMsgDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (hWnd)
	{
		...
		mouseStatus.x = point.x;
		mouseStatus.y = point.y;
		strcpy_s(mouseStatus.pTmp, "Right button click");
		msg.cbData = sizeof(MOUSESTATUS);
		msg.dwData = UM_MOUSESTATUS;
		msg.lpData = (PVOID)&mouseStatus;
